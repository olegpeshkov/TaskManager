...
class TaskManager:
    ...
    def add_task(self, task: Task):
        """
        Добавление задачи с обработкой ошибок и проверкой на дубликаты.
        """
        try:
            if not isinstance(task, Task):
                raise TypeError("Можно добавлять только объекты класса Task")

            # Проверяем, нет ли уже задачи с таким названием (без учёта регистра)
    ...
    def get_task_by_index(self, index: int):
        ...
        Возвращает задачу или None, если индекс некорректен.
        """
        try:
